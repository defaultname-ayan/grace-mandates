...
| **3:05** | Audit row showing `integrity_blocked` | "Case C. The baseline does the natural naive thing — the payment failed, charge it again — and walks into the eMandate confirmation window again and again. Razorpay's own docs say that confirmation can take more than 24 hours. The guard blocks every one -- the exact count is in the report." |
